...
impl CollectorContext for SSCollector {
    fn new() -> Self {
        SSCollector {
            ...
            _vs: RawPageAllocator::new(0 as *mut c_void, PLAN.get_vs()),
    ...
    fn alloc_copy(&mut self, _original: ObjectReference, bytes: usize, align: usize, offset: isize, allocator: AllocationType) -> Address {
        match allocator {
            // ::plan::Allocator::Los => self.vs.alloc(bytes, align, offset),
    ...
    fn collection_phase(&mut self, _tls: *mut c_void, phase: &Phase, primary: bool) {
        match phase {
            &Phase::Prepare => { self.ss.rebind(Some(PLAN.tospace())) }
            &Phase::StackRoots => {
    ...
    fn post_copy(&self, object: ObjectReference, _rvm_type: Address, _bytes: usize, _allocator: ::plan::Allocator) {
        CopySpace::post_copy(object)
    }
}
