...
impl Plan for NoGC {
    ...
    fn new() -> Self {
        NoGC {
            ...
            unsync: UnsafeCell::new(NoGCUnsync {
                ...
                cos: CodeSpace::new("cos", true, VMRequest::discontiguous()),
                total_pages: 0,
            }
            ),
        }
    ...
    fn bind_mutator(&self, tls: *mut c_void) -> *mut c_void {
        ...
        Box::into_raw(Box::new(NoGCMutator::new(
            tls, &unsync.space, &unsync.los, &unsync.cos))) as *mut c_void
    }
    ...
    fn get_pages_used(&self) -> usize {
        let unsync = unsafe { &*self.unsync.get() };
        unsync.space.reserved_pages() + unsync.los.reserved_pages() + unsync.cos.reserved_pages()
    }
    ...
    fn is_mapped_address(&self, address: Address) -> bool {
        ...
        if unsafe {
            ...
            unsync.cos.in_space(address.to_object_reference())
        } {
