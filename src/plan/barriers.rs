...
impl<E: ProcessEdgesWork> Barrier for ObjectRememberingBarrier<E> {
    ...
    fn flush(&mut self) {
        ...
        debug_assert!(
            !self.mmtk.scheduler.is_bucket_activated(WorkBucketStage::Final),
            "{:?}",
            self as *const _
        );
        if !modbuf.is_empty() {
            self.mmtk.scheduler.add_work(WorkBucketStage::Closure, ProcessModBuf::<E>::new(modbuf, self.meta));
        }
    }
