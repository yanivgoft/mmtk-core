...
impl ActivePlan<JikesRVM> for VMActivePlan {
    ...
    unsafe fn collector(tls: OpaquePointer) -> &'static mut <SelectedPlan as Plan<JikesRVM>>::CollectorT {
    ...
    // XXX: Are they actually static
    unsafe fn mutator(tls: OpaquePointer) -> &'static mut <SelectedPlan as Plan<JikesRVM>>::MutatorT {
        let thread: Address = unsafe { mem::transmute(tls) };
    ...
    fn get_next_mutator() -> Option<&'static mut <SelectedPlan as Plan<JikesRVM>>::MutatorT> {
        loop {
